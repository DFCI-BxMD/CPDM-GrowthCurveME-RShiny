...
shinyServer(function(input, output, session) {
  ...
  data <- eventReactive(input$data_file, { 
    ...
    if (!all(c("cluster", "time", input$growth_metric_name) %in% colnames(df))) {
      stop("The file must contain 'cluster', 'time', and the specified 'growth_metric' column.")
    }
    ...
    switch(input$plot_type,
           "Growth vs Time \nwith Model Predictions" = {
             growth_vs_time_plot(
               growth_model_summary_list = model_summary(),
               plot_type = 2,
               growth_metric_name = paste0("Growth Metric (", input$growth_metric_name, ")"),
               time_name = paste0("Time (", input$time_unit, ")"),
               cluster_name = paste0("Cluster (", input$cluster_name, ")"),
           ...
           "Growth vs Time \nwith Model Predictions by Cluster" = {
             growth_vs_time_plot(
               ...
               time_name = paste0("Time (", input$time_unit, ")"),
               cluster_name = paste0("cluster (", input$cluster_name, ")"),
               plot_title = "Growth vs Time \nwith Model Predictions by Cluster", #"Confluency vs Time with Cluster-Level Predictions",
               x_limits = c(input$x_min, input$x_max),  
               n_x_axis_breaks = input$n_x_axis_breaks,  
               y_limits = c(input$y_min, input$y_max),  
               n_y_axis_breaks = input$n_y_axis_breaks, 
  ...
  output$download_sample <- downloadHandler(
    ...
    content = function(file) {
      file.copy("/Users/ledia/Desktop/CPDM_Rshinny_Growth Curve/exp_mixed_data.csv", file)
    }
  )
  ...
  output$download_report <- downloadHandler(
    ...
    content = function(file) {
      ...
      # Create a subdirectory for PNG files and svg files
      png_dir <- "png"
      ...
      rmarkdown::render(
        ...
        params = list(
          ...
          report_title = "GrowthCurveME Analysis Report",
      ...
      plot_01 <- growth_vs_time_plot(
        ...
        plot_type = 2,
        growth_metric_name = paste0("Growth Metric (", input$growth_metric_name, ")"),
        time_name = paste0("Time (", stringr::str_to_title(input$time), ")"),
